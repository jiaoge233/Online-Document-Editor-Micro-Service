...
import (
	...
	"gateway/backend/internal/collab"
...
func NewConn(ws *websocket.Conn, hub *Hub, docID string, userID uint64, username string, svc collab.Service) *Conn {
	return &Conn{ws: ws, hub: hub, docID: docID, userID: userID, username: username, send: make(chan OutboundMessage, 32), svc: svc}
}
...
func (c *Conn) handleOpSubmit(ctx context.Context, msg OpSubmitMessage, authorID uint64) {
	_, err := c.svc.Submit(ctx, msg.DocID, authorID,
	...
	c.SendMessage_Enqueue(OpSubmitMessage{Type: "op_applied", DocID: msg.DocID, BaseRevision: msg.BaseRevision, CurrentRevision: curr_revision, ClientId: msg.ClientId, ClientSeq: msg.ClientSeq})
	//c.hub.BroadcastAppliedOp(msg.docId, toOpApplied(applied))
}
...
func (c *Conn) readLoop(ctx context.Context) {
	...
	for {
		...
		case "heartbeat":
			...
			members, err := c.hub.presence.GetMembers(ctx, c.docID)
			if err != nil {
				log.Printf("get members error: %v", err)
			}
			for _, member := range members {
				c.send <- ServerMessage{Type: "presence", Content: fmt.Sprintf("User %d is online", member)}
			}
		...
		case "register":
			...
			c.hub.presence.AddMember(ctx, c.docID, userId, username, 600*time.Second)
			c.send <- ServerMessage{Type: "register", Content: "User " + strconv.FormatUint(userId, 10) + " registered"}
		case "login":
			userId := clientMessage.UserID
			// password := clientMessage.Password
			username := clientMessage.Username
			c.hub.presence.AddMember(ctx, c.docID, userId, username, 600*time.Second)
			c.send <- ServerMessage{Type: "login", Content: "User " + strconv.FormatUint(userId, 10) + " logged in"}
		...
		case "createDocument":
			...
			//SetDocID(c, docID)
			userId, err := c.svc.GetUserID(ctx, username)
			if err != nil {
				log.Printf("get user id error: %v", err)
				c.send <- ServerMessage{Type: "error", Content: "USER_NOT_FOUND"}
				return
			}
			c.svc.CreateDocument(ctx, userId, docTitle)
		...
		case "setCursor":
			b, _ := json.Marshal(clientMessage.Range)
			err := c.hub.presence.SetCursor(ctx, c.docID, c.userID, b, 600*time.Second)
			if err != nil {
				log.Printf("set cursor error: %v", err)
			}
			c.hub.BroadcastCursor(c.docID, c.userID, clientMessage.Range)
			c.send <- ServerMessage{Type: "cursor", Content: fmt.Sprintf("Cursor set for user %d", c.userID)}

		case "getCursor":
			cursor, err := c.hub.presence.GetCursor(ctx, c.docID, c.userID)
			log.Printf("get cursor: %s", string(cursor))
			if err != nil {
				log.Printf("get cursor error: %v", err)
			}
			c.send <- ServerMessage{Type: "getCursor", Content: string(cursor)}
